...
class SpineHead(GenericSpineDomain):
    ...
    def d_c(self, n, r, c_r, charge_number):
        ...
        return charge_number * self.CONST_EBT * c_r  * (
                   E_rn + (r - r_n) / gmpy2.square(r) / eps * gmpy2.square(r_n) * excess_charge
               )
    ...
    def compute_electric_field(self, n):
        ...
        E_r(r_{n+1}) = E_r(r_n) * r_n^2 / r_{n+1}^2 + 1 / eps / r_{n+1}^2 * (r_{n+1} - r_n) (r_n^2 * rho(r_n)+ r_{n+1}^2*rho(r_{n+1})) / 2
        ...
        trap_term_pos = self.CONST_e * self.charge_number_c_pos * (gmpy2.square(r_n) * self.concentration_positive[n] + 
                                                                 gmpy2.square(r_np1) * self.concentration_positive[n + 1]) / mpfr('2')
        
        trap_term_neg = self.CONST_e * self.charge_number_c_neg * (gmpy2.square(r_n) * self.concentration_negative[n] + 
                                                                 gmpy2.square(r_np1) * self.concentration_negative[n + 1]) / mpfr('2')
        
        trap_term_back = self.CONST_e * self.charge_number_c_back * (gmpy2.square(r_n) * self.concentration_background[n] + 
                                                                   gmpy2.square(r_np1) * self.concentration_background[n + 1]) / mpfr('2')
        ...
        self.electric_field_pos[n+1] = gmpy2.square(r_n) / gmpy2.square(r_np1) * e_n_pos * eps_n / eps_np1 + 1 / eps_np1 / gmpy2.square(r_np1) * (r_np1 - r_n) * trap_term_pos
        self.electric_field_neg[n+1] = gmpy2.square(r_n) / gmpy2.square(r_np1) * e_n_neg * eps_n / eps_np1 + 1 / eps_np1 / gmpy2.square(r_np1) * (r_np1 - r_n) * trap_term_neg
        self.electric_field_back[n+1] = gmpy2.square(r_n) / gmpy2.square(r_np1) * e_n_back * eps_n / eps_np1 + 1 / eps_np1 / gmpy2.square(r_np1) * (r_np1 - r_n) * trap_term_back
    ...
    def compute_e_field_membrane_charge_in(self):
        ...
        the the field drops with r_n^2/r_{n+1}^2
        """
        ...
        for i in range(self.res_in+1, self.res_in+self.res_mem+self.res_out-2):
            ...
            # compute field
            self.electric_field_mem_in[i] = gmpy2.square(r_j) / gmpy2.square(r_i) * e_mem_in_j * eps_j / eps_i
    ...
    def compute_e_field_membrane_charge_out(self):
        ...
        for i in range(self.res_in+self.res_mem, self.res_in+self.res_mem+self.res_out-2):
            ...
            # compute field
            self.electric_field_mem_out[i] = gmpy2.square(r_j) / gmpy2.square(r_i) * e_mem_out_j * eps_j / eps_i
    ...
    def compute_cumulative_charge(self):
        ...
        unit [Q] = C/m
        Q(r) = \int_0^r 4*pi*sigma(r) r^2 dr
        """
        ...
        for i in range(self.res_in,  self.res_in+self.res_mem+self.res_out-2):                          
            self.cumulative_mem_in[i] = mpfr('4') * self.CONST_PI * gmpy2.square(self.grid_points[self.res_in]) * self.sigma_surf_in
        ...
        for i in range(self.res_in+self.res_mem-1,  self.res_in+self.res_mem+self.res_out-2):    
            self.cumulative_mem_out[i] = mpfr('4') * self.CONST_PI * gmpy2.square(self.grid_points[self.res_in+self.res_mem-1]) * self.sigma_surf_out
        ...
        for i in range(1, self.res_in+self.res_mem+self.res_out-2):
            # positive charges
            self.cumulative_positive[i] = self.cumulative_positive[i-1]+ mpfr('4') * self.CONST_PI * ((self.grid_points[i] - self.grid_points[i-1]) / mpfr('2') * 
                                           (gmpy2.square(self.grid_points[i]) * self.concentration_positive[i] * self.charge_number_c_pos + 
                                            gmpy2.square(self.grid_points[i-1]) * self.concentration_positive[i-1]* self.charge_number_c_pos)) * self.CONST_e
            # negative charges
            self.cumulative_negative[i] = self.cumulative_negative[i-1] + mpfr('4') * self.CONST_PI * ((self.grid_points[i] - self.grid_points[i-1]) / mpfr('2') * 
                                           (gmpy2.square(self.grid_points[i]) *   self.concentration_negative[i]* self.charge_number_c_neg + 
                                            gmpy2.square(self.grid_points[i-1]) * self.concentration_negative[i-1]* self.charge_number_c_neg)) * self.CONST_e
            # background charges
            self.cumulative_background[i] = self.cumulative_background[i-1] + mpfr('4') * self.CONST_PI * ((self.grid_points[i] - self.grid_points[i-1]) / mpfr('2') * 
                                           (gmpy2.square(self.grid_points[i]) *   self.concentration_background[i]* self.charge_number_c_back + 
                                            gmpy2.square(self.grid_points[i-1]) * self.concentration_background[i-1]* self.charge_number_c_back)) * self.CONST_e
    ...
    def compute_pnp_lhs(self):
        ...
        for i in range(1, self.res_in - 1):
            self.pnp_lhs[i] = d2r(self.grid_points, self.electric_potential, i) + mpfr('2') / self.grid_points[i] * d1r(self.grid_points, self.electric_potential, i, method='cd')
